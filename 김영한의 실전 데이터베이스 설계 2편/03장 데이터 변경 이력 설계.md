...
- 변경된 컬럼만 저장하는 방식, 변경되지 않은 컬럼은 저장하지 않는다.
...
2. 무엇이 변경되었는지 명확
    ...
    > **old_value**, **new_value**가 있기 때문

#### 단점

1. 특정 시점의 전체 데이터를 복원하기 어려움

    > 특정 시점을 기준으로 **column_name**을 다 찾아야함

2. 통계 쿼리 작성의 어려움

    > 특정 시점을 기준으로 데이터 집계 어려움

3. 타입 정보의 손실

    > 모든 데이터는 VARCHAR로 저장되므로 숫자 또는 날짜형의 데이터 손실 발생
...
1. 기본 전략
    ...
    > 기본적으로 **전체 행 스냅샷** 방식을 선택한다. 구현이 가장쉽고 간편하면서 안전함

2. 예외 상황

    > 데이터가 수억 건 이상 쌓인다면 **컬럼 단위 변경 로그 테이블**을 선택할 수도 있겠지만, 차라리 **전체 행 스냅샷** 방식을 사용하면서 `cold storage`를 추가적으로 사용하는게 좋을거 같음
