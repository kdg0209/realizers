...
1. 공통 코드를 데이터베이스에 설정하는 경우 매번 JOIN이 필요할 수 있음
2. 도메인별 코드를 하나의 테이블에서 관리하면 코드가 많아졌을 때 파악하기 힘듬
3. 도메인이 다르지만 비슷한 코드가 생겼을 때 어디서 사용되는지 역추적하기 힘듬
4. 코드값이 충돌할 수 있음
...
`해결책 1` 에서는 공통 코드가 그룹화되어 있지 않아 문제가 발생

<br>

### 장점

- 상세 코드 테이블에 중복된 코드가 있더라도 그룹 코드가 다르기 때문에 파악이 쉬워짐

<br>

### 단점

- 비지니스가 복잡해질수록 쿼리가 복잡해짐
...
## 해결책 1. 상세 코드 테이블에 컬럼 추가
 
### 장점

- 추가 속성이 정말 적다면 간단하게 문제를 해결할 수 있음

<br>

### 단점

- 컬럼이 무엇을 의미하는지 파악하기 힘듬
- 속성이 계속 추가될 경우 컬럼이 무자비하게 추가될 수 있음
- 속성을 Json 컬럼을 사용하여 저장해도 되겠지만 경우에 따라 속성으로 검색을 해야하는 경우 프레임워크(ex: Spring Boot)에서 Native 쿼리를 사용해야함
...
## 해결책 2. EAV(Entity-Attribute-Value)방식
